            ...
            if (!dataSet.getGradientColors().isEmpty()) {
                 mRenderPaint.setShader(
                    new LinearGradient(
                        ...
                        buffer.buffer[i],
                        buffer.buffer[i + 1],
                        dataSet.getGradientColor(i / 4).getStartColor(),
                        dataSet.getGradientColor(i / 4).getEndColor(),
                        android.graphics.Shader.TileMode.MIRROR));
            }


            canvas.drawRect(buffer.buffer[i], buffer.buffer[i + 1], buffer.buffer[i + 2],
                ...
                if (!dataSet.isStacked()) {
                    for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                        ...
                        if (dataSet.isDrawValuesEnabled()) {
                            drawValue(
                                    canvas, formatter.getBarLabel(entry), x,
                    ...
                    while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                        ...
                        if (values == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (dataSet.isDrawValuesEnabled()) {
                                    drawValue(canvas, formatter.getBarStackedLabel(val, entry), x, y, color);
                                }
